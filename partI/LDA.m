...
%     E_is = {};
%     for it = n_eis'
%         E_is = [E_is, 1/it * ones(it)];
%     end
    
    M = blkdiag(E_is{:});
    
    i_minus_m = (eye(length(labels)) - M);
    [V_w, Lambda_w] = eigs(i_minus_m * central_data * central_data' * i_minus_m, size(central_data, 1) - (size(unique(labels), 1) + 1));
    U = central_data' * i_minus_m * V_w * inv(Lambda_w);
